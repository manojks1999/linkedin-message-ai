...
class Client(object):
    ...
    def _do_authentication_request(self, username: str, password: str):
        ...
        print("pajdfdsf", payload, self.session.cookies)
        res = requests.post(
            f"{Client.LINKEDIN_BASE_URL}/uas/authenticate",
            data=payload,
            cookies=self.session.cookies,
        ...
        counter = 0
        while 'challenge_url' in data and data['challenge_url'] != "" and counter < 10:

            res = requests.get(
                f"{data['challenge_url']}",
                # data=payload,
                cookies=self.session.cookies,
                headers=Client.AUTH_REQUEST_HEADERS,
                proxies=self.proxies,
            )
            print("kjdjdjdjdds", res, counter)
            res = requests.post(
                f"{Client.LINKEDIN_BASE_URL}/uas/authenticate",
                data=payload,
                cookies=self.session.cookies,
